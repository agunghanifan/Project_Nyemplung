import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

Color blackColor = const Color(0xff000000);
Color greenColor = const Color(0xff5DB075);
Color whiteColor = const Color(0xffFFFFFF);
Color greyColor = const Color(0xffF6F6F6);
Color greyTextColor = const Color(0x00bdbdbd);

TextStyle titleTextStyle = GoogleFonts.inter(
    color: blackColor, fontWeight: FontWeight.bold, fontSize: 30);
TextStyle buttonTextStyle = GoogleFonts.inter(
    color: whiteColor, fontWeight: FontWeight.bold, fontSize: 16);
TextStyle buttonTextTextStyle = GoogleFonts.inter(
    color: greenColor, fontWeight: FontWeight.bold, fontSize: 16);
TextStyle commonTextStyle = GoogleFonts.inter(color: blackColor, fontSize: 14);
TextStyle greyTextStyle = GoogleFonts.inter(color: Colors.blue, fontSize: 14);

double edge = 16;
